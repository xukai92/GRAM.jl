...
function build_chain(D_in, D_h, D_out, σ, σ_last)
    n_layers = length(D_h)
    return Chain(
...
function Generator(D_z::Int, D_h::AbstractVector{Int}, D_x::Int, σ, σ_last)
    σ, σ_last = parse_op(σ), parse_op(σ_last)
    f = build_chain(D_z, D_h, D_x, σ, σ_last)
    return Generator(D_z, f)
end

function Distributions.rand(rng::AbstractRNG, g::Generator, n::Int)
...
function Projector(D_x::Int, D_h::AbstractArray{Int}, D_fx::Int, σ)
    σ = parse_op(σ)
    f = build_chain(D_x, D_h, D_fx, σ, identity)
    return Projector(f)
end
